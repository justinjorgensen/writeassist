...
When running multi-agent review (`/review-chapter`):
1. **Use separate, named Task agents** - never simulate multiple critics in one Task.
2. **Launch the seven named gating critics in parallel** - they run in a single response, each in its own clean context.
3. **Each agent gets clean context** - no contamination between analyses.
4. **The four-tier engine decides** - a panel gate plus a weighted gate (Strong Pass / Pass / Needs Work / Fail), not a numeric score chase.
5. **See `docs/ARCHITECTURE.md`** for how the review engine and the seven critics fit together.
