...
- [x] **R-22: Purge phantom agents and commands.**
...
- [x] **R-23: Remove prior-project residue.**
...
- [x] **R-24: Delete fabricated metrics; archive obsolete v1 docs (D9).**
...
- [x] **R-30: Add frontmatter and input contracts to all commands.**
...
- [x] **R-31: Batch commands compose the single-chapter commands (D11).**
...
- 2026-07-03: R-21 done. Global kebab-case pass over .claude/, CLAUDE.md, project-config.md, MIGRATION.md: 24 snake_case agent names converted (26 files touched), rules_enforcer corrected to rule-enforcer, all six batch-review-and-fix sites now batch-review-and-revise. Resolver check (snake token whose kebab form matches an agent/command file) returns zero unresolved names; pseudocode function names left as code.
- 2026-07-03: R-22 done. reader_analyst to beta-reader-sim; competitive_positioning to market-analyst; theme-explorer to thematic-guide; genre-specialist and foreshadowing integration lines deleted; batch-operations' 10-name phantom roster replaced with the D6 critic panel; Story Compendium Manager agent to the /story-compendium-manager command; /restore-chapter to git-based rollback; compile-manuscript to book-cleanup; query-prep/market-ready/review-sim/adaptation-assess mapped to send-query-letter, review-chapter, or direct-Task notes. D10 applied: all /ultrareview text now /code-review ultra (CLAUDE.md, MIGRATION.md, SKILL.md, ultrareview-gate.md content, help.html). Deviations: the FILE ultrareview-gate.md keeps its name (the plan itself references it in R-24/R-42); help.html anchor ids kept; MIGRATION.md path refs briefly mangled by the rename and repaired.
- 2026-07-03: R-23 done. continuity-checker's three worked detections rewritten with [Protagonist]/[Child A]/[Child B]/[Example Book] placeholders plus an explicit "placeholders, not story facts" note; transition-validator's EMP examples genericized ([Protagonist], [Antagonist], [Ally]); validate-transitions' Divine Replica mentions neutralized. simple-content-analyzer's hardcoded character list was already gone (deleted in R-20). Verify grep for Divine Replica/Elena/Dr. Chen over .claude/ returns nothing. Note: incidental Sarah/Marcus names in OTHER generic craft examples (dialogue samples etc.) are not story-fact assertions and were left; the verify grep does not cover them.
- 2026-07-03: R-24 done. Both v1 docs moved to .claude/docs/archive/ with deprecation headers (gitignore exception added so they stay under version control); system-guides/README.md rewritten to list what actually exists (review-engine.md located correctly, cron-setup.md and ultrareview-gate.md added, archive noted). Deleted every unmeasured number: CLAUDE.md Performance Characteristics trio and 10x lines, review-chapter ~30s claim, project-config 10x, cron-setup 78%, batch-execute-wrp CPU/Memory dashboard, batch-generate-wrp timing table, batch-review-and-revise Processing Speed block, Resource Usage dashboard, and "REDUCED by 78%"; auto-revise progress example renumbered off the flagged pattern. CLAUDE.md/project-config pointers to PARALLEL-EXECUTION-GUIDE now point to review-chapter's parallel-execution section. Verify grep clean outside archive/.
- 2026-07-03: R-30 done. All 27 command files (evidence said 28; actual count is 27) now open with YAML frontmatter (description everywhere; argument-hint where args exist; allowed-tools on the read-only set: compare-drafts, curate-chapters, dialogue-specialist, validate-transitions, workflow-guide, and review-chapter/smart-review). Every command states a No-argument behavior line; commands that lacked $ARGUMENTS got an explicit Target: $ARGUMENTS line. execute-wrp now resolves either a full path or a bare name against 05-wrp/ instead of blindly prepending. Deviation: review-chapter and smart-review keep Write in allowed-tools solely for the R-34 report path (.claude/state/reviews/), noted inline in each file.
- 2026-07-03: R-31 done. batch-generate-wrp, batch-execute-wrp, and batch-review-and-revise rewritten as thin loops over their single-chapter counterparts: resolve targets, invoke the single command (--no-plan / suppressed checkpoints in batch context), record-and-continue on failure, one batch report at .claude/state/batch-reports/. Deleted: Parallel-4 threading, -scores.md sidecars, resource dashboards, Continuous Monitoring Mode, the 3-band confidence-ladder copy (auto-revise's 4-band ladder is now the only one). Autonomy resolved in auto-revise-chapter.md: the iteration-3 checkpoint is explicitly suppressed when invoked by execute-wrp or any batch command. batch-operations.md survives as the pipeline reference (its phantom agents and 8.5 threshold were already fixed in R-22/R-10). Verify greps clean.
