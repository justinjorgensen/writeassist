...
Sync just that file. Use this for incremental updates after a chapter passes the review panel.
...
- Backup/draft files matching any deprecated versioning pattern: `*-backup-*.md`, `*-draft*.md`, `*-iteration-*.md`, anything under `versions/` or `archive/`
...
Report the offending files and lines, and point the author at the marker-strip steps in `auto-revise-chapter` / `write-scene`. A chapter with markers is not done; it must not reach readers.
