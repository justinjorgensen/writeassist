...
# status to stdout.
#
# Displays:  [model] | proj-name | chapter-active | total-words | em-dashes | last review decision (PASS/REVISE)
set -uo pipefail
...
if [[ -d "$review_dir" ]]; then
  ...
  if [[ -n "$latest_report" ]]; then
    last_review="$(grep -oiE 'Decision:[[:space:]]*(PASS|REVISE)' "$latest_report" | head -1 | grep -oiE 'PASS|REVISE' | tr 'a-z' 'A-Z' || echo '...')"
    [[ -n "$last_review" ]] || last_review="..."
  fi
fi
...
printf '%s[%s]%s %s %s│%s %s %s│%s %s words %s│%s %s %s│%s review:%s\n' \
  "$DIM" "$model" "$RESET" \
  "$proj_name" \
  "$DIM" "$RESET" "$latest_ch" \
  "$DIM" "$RESET" "$total_words" \
  "$DIM" "$RESET" "$emdash_str" \
  "$DIM" "$RESET" "$last_review"
