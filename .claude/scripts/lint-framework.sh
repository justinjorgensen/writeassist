...
for f in "$AGENTS_DIR"/*.md; do
  ...
  done < <(grep -rnw "$snake" .claude CLAUDE.md project-config.md integrations docs --include='*.md' 2>/dev/null \
...
# ---------------------------------------------------------------------------
BUILTINS='code-review|goal|help|clear|config|fast|schedule|loop|init|review|output-style'
...
while IFS= read -r line; do
  ...
  for cmd in $(printf '%s' "$rest" | grep -oP '(?<=[ `(>])/[a-z][a-z0-9-]{2,}(?=[ `.,)\x27]|$)' | sed 's|^/||' | sort -u); do
    ...
    [[ -f ".claude/commands/$cmd.md" || -f ".claude/skills/$cmd/SKILL.md" || -f "integrations/$cmd.md" ]] || finding "unresolved /command reference '/$cmd': $file"
  done
done < <(grep -rn ' /[a-z][a-z0-9-]\{2,\}\|`/[a-z][a-z0-9-]\{2,\}' .claude CLAUDE.md project-config.md integrations --include='*.md' 2>/dev/null \
...
while IFS= read -r hit; do
  finding "gate number outside review-engine.md: $hit"
           ...
           CLAUDE.md project-config.md .claude/commands .claude/skills .claude/agents .claude/docs integrations 2>/dev/null \
