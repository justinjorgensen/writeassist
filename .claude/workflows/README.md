...
| `import-book.js` | Onboards an existing draft: scouts the manuscript chapter by chapter, then reconciles findings into framework scaffolding. The scout pass reverse-engineers the story bible (compendium, characters, timeline, implied rules) from the prose; the reconcile pass diffs that against any existing config, surfaces every disagreement, and writes a conflict ledger of open questions for the author rather than guessing. Driven by the `/import-book` command. | args = a manuscript path string (`"02-Manuscript/"`) or `{ manuscript, config?, fast? }`. `config` is an existing config path to reconcile against; `fast: true` scouts without the deep timeline pass. Returns `{ storyBible, ledger:[...], conflicts, scouted }`. |
