...
**No-argument behavior:** If no argument is given: look for the most recent REVISE decision in `.claude/state/reviews/`; if none exists, list chapters in `02-Manuscript/` and stop.
...
## Worktree-Isolated Iteration (v2)

Each revision pass runs inside its **own git worktree** so the author can diff between iterations instead of trusting a single overwritten file.
...
### Iteration loop
For pass `N`:
1. Spawn a Task agent with `isolation: "worktree"` (or `cd` into `.worktrees/chapter-XX-pass-N`).
2. Apply the confidence-ladder fixes against that worktree's copy of the chapter.
3. Commit: `git -C .worktrees/chapter-XX-pass-N commit -am "auto-revise pass N"`.
4. Run `/review-chapter` inside the worktree.
5. If panel passes → fast-forward merge that branch into the working branch and remove the worktree.
6. If fails and N < MAX_ITERATIONS → create `revise/chapter-XX-pass-(N+1)` from the previous pass and loop.
...
- With markers (0.90-0.95): 45 fixes
- Suggestions (0.85-0.90): 22 fixes
- Skipped (<0.85): 44 fixes

Em Dash Removals (CRITICAL): 7 → All fixed ✓
```
