...
- **Final step (MANDATORY)**: once the author accepts the scene, strip the markers; verify `grep -n "\[SCENE ADDED" <chapter-file>` returns nothing. `sync-to-drive` refuses files that still contain them.
- Updates word count
