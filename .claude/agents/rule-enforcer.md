...
```markdown
[RULE VIOLATION: No head-hopping]
...
### Coordinates With
- **style-editor** - Enforces style rules
...
- Flag rule breaches in the body with `[RULE VIOLATION: rule-name]` tags (matching CLAUDE.md), then reflect them in `fixes` and the tier.
